...
| S5 | Several read queries have **no server-side auth check at all**, relying entirely on the `/admin` layout gate | `members.getMemberDetail`; all of `repCodes.ts`'s queries; `userAudit.getUserStatuses`/`getUserDetail`; `siteSettings.get`; `billing.getGroupMembersWithBillingStatus`/`getAccountBillingSummary`/`getSiteBillingSummary`/`getUpcomingBillingDates`; `commissions.getBrokerCommissions`; `devTools.seedCatalog`/`linkAdminAsMember` | Anyone who can reach the Convex deployment directly (not just through the Next.js UI) could call these without being an admin. Low risk in normal usage, but inconsistent with the rest of the codebase's own stated convention of gating at the function level. |
| S6 | `GET /api/clerk/users` only checks "is this person signed in," not "is this person an admin" | `src/app/api/clerk/users/route.ts` | Used by Admin Users' "Add Existing User" search and by User Lookup. Any signed-in Clerk user could call it directly and enumerate the Clerk user directory (names/emails/IDs). |
...
| B12 | **Revenue & Dispersal's own PDF route folder is empty** | `src/app/api/admin/invoice-calculator/group-pdf/` (no `route.ts` inside) | Dead scaffolding. The "Generate Invoice" wizard's PDF button actually opens the List-Bill Invoices PDF route instead. |
...
| G3 | **No audit-log entries for Rep Code CRUD** | `convex/admin/repCodes.ts` | Broker and team changes are now audited (see the broker workspace's Activity tab); creating/updating/revoking a rep code on the Rep Codes page still writes nothing to `adminAuditLog`. |
...
| # | What | Where | Note |
|---|---|---|---|
| D1 | `src/components/admin/BrokersAdmin.tsx` | Unused | The live "Brokers" pages are in `src/components/admin/brokers/`. This file isn't imported anywhere. |
...
| D3 | `convex/admin/hardDeleteMember`'s UI reachability | N/A — not a bug, a routing note | `hardDeleteMember` is only reachable from **User Lookup**, not from the Members page (which only soft-deletes via `removeMember`). Intentional, but easy to go looking for it in the wrong place. |
