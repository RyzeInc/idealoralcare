...
**When you'd do this:** A new employer relationship is ready to start enrolling members.

**Before you begin:**
- Know which **Site** (almost always the one existing "Ideal Health" site) and which **Account** this group belongs to. If the Account doesn't exist yet, create it first (Step 2).
...
1. Go to **Hierarchy** (`/admin/hierarchy`) — sidebar label "Hierarchy," page header "Sites, Accounts & Organizations."
2. Confirm the parent **Account** exists:
   - Click the **Accounts** tab and search for it.
   - If it doesn't exist: click **Create Account**, pick the Carrier (Site), enter a unique Slug, choose an Account Type and Billing Model, and submit.
3. Click the **Organizations** tab → **Create Organization**.
4. Fill in the form:
   - **Carrier** → the Site.
   - **Account** → the one you confirmed or created in step 2 (the dropdown filters to that carrier).
...
5. Click **Create**.
6. Confirm the new Group appears in the Organizations table with the correct Org Code and Account.
...
- **Wrong Account selected** — you can Edit the Group afterward to fix it; this doesn't cascade-delete anything.
...
- [SOP-004](SOP-004-onboard-broker-agency-and-reps.md) — if the broker/agency itself also needs to be set up in the separate Brokers / commission chain.
