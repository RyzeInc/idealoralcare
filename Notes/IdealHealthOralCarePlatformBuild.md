...
Member ID card generation — Create a simple member card view/PDF with: member name, 9-digit member ID, plan name, effective date, Dental Discount Network network info, Dial Care access info, Toothlens smart check link. Make it downloadable from the dashboard and emailable.
...
Verification
Agent 1 done when: npm run build succeeds with no type errors; the site renders with dynamic branding from Convex; only the single $15/$13 Oral Health Plan appears in the catalog; no Ideal/Nexus references remain.
